...
import { getRecordLast, getAllDevice, adjustThreshold } from "../../services/userService";
...
export default function ThresholdScreen({navigation}) {
    ...
    useEffect(() => {
        const getAllDevices = async () => {
            ...
            if (res)
            {
                const tempData = JSON.parse(JSON.stringify(res.data)).map((item) => {
                    if (item.name == "humid")
                        setThresHumid(item.threshold.toString());
    ...
    const handleConfirm = async () => {
        ...
        {
            var currThreshold = "";
            if (devices[i].name == "humid")
                currThreshold = thresHumid;
            else if (devices[i].name == "light")
                currThreshold = thresLight;
            else if (devices[i].name == "temp")
                currThreshold = thresTemp;
                ...
                <DataTable>
                    <DataTable.Row>
                        <DataTable.Cell style={{alignItems: "center", justifyContent: "center"}}>
                            <Text style={{fontSize: 23, color: "#3CAF58"}}>Ngưỡng dữ liệu</Text>
                        </DataTable.Cell>
                    </DataTable.Row>
                            ...
                            return (
                                <DataTable.Row key={index} style={{marginLeft: "5%"}}>
                                    <DataTable.Cell style={{ flex: 1 }}>
                                        <Text style={
                                            item.name == "humid" && overHumid? { color: "red" }
                                            :
                                            (
                                    ...
                                    <DataTable.Cell style={{ flex: 1 }}>
                                        <TextInput
                                            ...
                                            value={item.name == "humid"? thresHumid : (item.name == "light"? thresLight : thresTemp)}
                    ...
                    <DataTable.Row>
                        <DataTable.Cell style={{alignItems: "center", justifyContent: "center", flex: 1}}>
                            <Button
                                ...
                                onPress={() => {
                                    ...
                                    {
                                        if (handleConfirm())
                                            Alert.alert("Thành công", "Ngưỡng dữ liệu đã được cập nhật!");
                                        else
